"""
self_update — Auto-aggiornamento
==================================
Controlla su GitHub se c'e' una nuova versione,
scarica il nuovo eseguibile e lo installa.

Usa le GitHub Releases API per confrontare la
versione corrente con l'ultima disponibile.
"""
...
def _parse_remote_version(release_data: dict) -> Optional[str]:
    """Estrae la versione dal tag della release (es. 'v1.0.1' -> '1.0.1')."""
...
def check_for_updates(timeout: int = TIMEOUT) -> Tuple[bool, str, Optional[dict]]:
    ...
    Restituisce: (disponibile, messaggio, dati_release)
    """
    try:
...
def download_update(release_data: dict, timeout: int = 120) -> Tuple[bool, str]:
    """
    Scarica l'ultimo eseguibile dalla release.
    Cerca l'asset .exe tra i file della release.
    """
...
def apply_update(downloaded_exe: str) -> Tuple[bool, str]:
    """
    Applica l'aggiornamento: sostituisce l'eseguibile corrente
    con quello scaricato. Crea un batch per il rename al prossimo avvio
    (per evitare problemi con file in uso).
    """
    ...
    # Crea script di sostituzione al prossimo avvio
